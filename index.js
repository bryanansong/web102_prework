...
function addGamesToPage(games) {
    ...
    for ( var game in games ) {
        ...
        <text>Name: ${games[game].name}<br></text>
        <text>Description: ${games[game]["description"]} </text>
        `;
...
contributionsCard.innerHTML = `<text>${contributions.toLocaleString("en-US")}</text>`
...
gamesCard.innerHTML = GAMES_JSON.reduce( (acc, game) => ( acc+=1 ), 0).toLocaleString('en-us');
...
const totalNumberOfGames = GAMES_JSON.reduce( (acc, game) => (acc+=1), 0);
// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${amountRaised.toLocaleString('en-US')} has been raised for 
${totalNumberOfGames} ${ totalNumberOfGames-numberOfUnfundedGames != 1 ? "games" : "game"}. 
Currently, ${numberOfUnfundedGames} ${ totalNumberOfGames-numberOfUnfundedGames != 1 ? "games" : "game"} 
